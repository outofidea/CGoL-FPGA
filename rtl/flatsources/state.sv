...
module state #(
    ...
    logic ignore_this_clk_calc_ready;  // ! cuz im dumb
        ...
        if (rst) begin
            ...
            case (state)
                ...
                WAIT_CALCULATION: begin
                    display_buf_change_ready   <= 1'b0;
                    ignore_this_clk_calc_ready <= 1'b0;
                    if (calc_done && !ignore_this_clk_calc_ready) begin
                        state <= WAIT_DISPLAY;
                    end
                    calc_done_ack <= 0;
                end
                ...
                WAIT_DISPLAY_READY: begin
                    if (!display_buf_change_ack_sync_2) begin
                        calc_done_ack              <= 1'b1;
                        ignore_this_clk_calc_ready <= 1'b1;
                        display_buf_change_ready   <= 1'b0;
    ...
    always_comb begin : buffer_mux_comb
        ...
        unique case (state) inside
            ...
            WAIT_DISPLAY, WAIT_DISPLAY_READY: begin
                if (current_display_buffer == 1) begin  //! no writes allowed 
                    buf1_calc_we           = 1'b0;
                    buf1_calc_addr         = calc_cell_addr;
                    buf1_calc_data_in      = calc_dat_in;
